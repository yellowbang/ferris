import banliang from './banliang.jpg';
import eshit from './eshit.jpg';
import ferris from './ferris.jpeg';
import jiehun from './jiehun.jpeg';
import kayak from './kayak.jpeg';
import mianmo from './mianmo.jpeg';
import shou from './shou.jpg';
import smellFoot from './smellFoot.png';
import wenquan from './wenquan.jpg';
import withJuan from './withJuan.jpg';
import xiao from './xiao.jpg';
import zhoulianjinian from './zhoulianjinian.jpeg';

export default [
    {
        src: ferris,
        descr: '主题的灵感是黎自我地上次坐的大型摩天轮。自从上次坐摩天轮的时候我就想写一个网站比你，宜家终于有机会。',
    },
    {
        src: shou,
        descr: '好中意你衣个又瘦又清纯的样子。虽然你宜家肥到可能连家姐都唔认得，不过我依然甘中意你。',
    },
    {
        src: mianmo,
        descr: '人不是最靓的时候最好睇的，而是在最幸福最傻的时候才是最好睇。所以好中意同你一齐扮鬼脸。你睇你衣张白得黎几靓。',
    },
    {
        src: smellFoot,
        descr: '另一件傻事就系可以同你玩塞脚趾游戏。无论几累都好，只要可以整蛊你，我就精力充沛。你今日被整蛊左未？',
    },
    {
        src: xiao,
        descr: '好想保护着你衣种又天真又傻的笑容。每次睇到你衣张笑得甘傻的相，我都会自然就会跟著笑。',
    },
    {
        src: withJuan,
        descr: '费事你睇得甘过瘾，所以插一张同阿娟的合照，激死你。',
    },
    {
        src: wenquan,
        descr: '第一次泡真温泉，实在太正啦。多谢你带比我甘多新奇好玩带经历。好想同你周围旅游，实现更多的第一次。',
    },
    {
        src: kayak,
        descr: '你扮出痕力洼船的样几奸诈。你上次话好想每年都去 kayak 一次的时候，我有一种老怀安慰的感动。讲真，我地好耐都无一齐运动啦。',
    },
    {
        src: eshit,
        descr: '要封女神的话，毫无疑问你一定屙屎女神。无论从屙屎的姿态，或屙出黎的屎的形态，你都系最 Perfect 的。',
    },
    {
        src: banliang,
        descr: '我真系好中意做兄弟姐妹，可以见证着朋友的幸福真系好幸福。更幸福的系可以同你一齐参与，一齐经历这种幸福的时刻。',
    },
    {
        src: jiehun,
        descr: '确认过眼神，你是个小肥人。虽然你又矮又开始肥，久唔久又会唔知点解成个焖姜甘。但我依然甘爱你。',
    },
    {
        src: zhoulianjinian,
        descr: '同你一齐的总有特别的日子。今年的周年纪念是史上最短周年纪念，短得黎几得意几特别。同你一齐的每一日都系值得纪念的日子。',
    },
]
