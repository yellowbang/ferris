import before_born from "./before_born.jpeg";
import new_born from "./new_born.jpeg";
import day_100 from "./day_100.jpeg";
import burp from "./burp.jpeg";
import boss from "./boss.jpeg";
import sleep_bon from "./sleep_bon.jpeg";
import sleep_jojo from "./sleep_jojo.jpeg";
import chair_200 from "./chair_200.jpeg";
import butterfly from "./butterfly.jpeg";
import cosplay from "./cosplay.jpeg";
import yellow from "./yellow.jpeg";
import zoo from "./zoo.jpeg";
import sick from "./sick.jpeg";
import all_family from "./all_family.jpeg";
import all2 from "./all2.jpeg";
import one_year from "./one_year.jpeg";

export default [
  {
    src: all2,
    descr: "第一次全家人影艺术照。希望以后每年都可以影一张。",
  },
  {
    src: one_year,
    descr: "Boyeer仔话：“我日头喜欢爸爸多滴，夜晚妈妈才是我的最爱。”",
  },
  {
    src: sick,
    descr: "你第一次发烧的样。你嫲嫲好紧张你啊。发烧好啊，有助你未来的健康啊。",
  },
  {
    src: cosplay,
    descr:
      "第一次全家cosplay。主题系千金大小姐成功追到勤劳车夫，然后生下得意 Boyeer。",
  },
  {
    src: yellow,
    descr: "终于稳到一张你着黄色衫的相。唔知你以后最中意的颜色系咩色呢？",
  },
  {
    src: zoo,
    descr: "爸爸妈妈同你第一次去动物园睇灯饰。见到闪闪发光的野，你中唔中意啊？",
  },
  {
    src: butterfly,
    descr:
      "纯熟恶搞，你爸爸妈妈都有类似的相嘎。唔知你会唔会第日都系一个有趣而且搞笑的人呢。",
  },
  {
    src: sleep_jojo,
    descr:
      "每次见到你同妈妈好安逸甘睡觉的时候，我就觉得好幸福。好想你地每日都安稳的生活。到你长大成家立业的时候都要好好保护你的家人啊。也许到时候你就体会到我现在的心情。",
  },
  {
    src: chair_200,
    descr:
      "之所倚拣这张相，纯粹系为了hight light 衣张$200蚊的凳。。醒目啊，识得喇少少💩系张凳上。你妈妈想退都退唔到。",
  },
  {
    src: all_family,
    descr: "全家福啊。因为有你，全家福更加甜美。因为有你，全家生活有趣又得意",
  },
  {
    src: day_100,
    descr:
      "终于100日生日啦。好不容易的前3个月，每天都系喂奶，换尿片中不断的循环。都真是不容易啊。",
  },
  {
    src: boss,
    descr:
      "Boyeer 的第一张艺术照。嚣得黎又几得意。你影依张相的时候，我地笑左好耐。太搞笑了。",
  },
  {
    src: new_born,
    descr:
      "Welcome to the World。终于见到你啦。我地第一张的全家福。你妈妈几累。",
  },
  {
    src: before_born,
    descr:
      "妈妈做紧最后的努力。我地都期待紧你的出现啊。妈妈生你果日痛左成日嘎。心痛啊。",
  },
];
